package com.qperior.referenceapp.srvjava;
...
public class HelloServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try (OutputStreamWriter writer = new OutputStreamWriter(response.getOutputStream(), "UTF-8")) {
			...
			if (conn != null) {
				...
				writer.write("\n\nTest data from table 'Books':\n");
				writer.write(getTestData(conn));
			} else {
				writer.write("no");
			}
		} catch (SQLException e) {
	...
	private String getTestData(Connection conn) throws SQLException {
		String testData = "";
		PreparedStatement prepareStatement = conn.prepareStatement("SELECT * FROM MY_BOOKSHOP_BOOKS;");
		ResultSet resultSet = prepareStatement.executeQuery();
		int column1 = resultSet.findColumn("ID");
		int column2 = resultSet.findColumn("TITLE");
		int column3 = resultSet.findColumn("STOCK");
		while (resultSet.next()) {
			testData += resultSet.getString(column1) + " ";
			testData += resultSet.getString(column2) + " ";
			testData += resultSet.getString(column3) + " \n";
		}
		return testData;
	}
